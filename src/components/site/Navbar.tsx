  ...
  return (
    <header className="fixed inset-x-0 top-4 z-50 px-4 md:px-8">
      <motion.nav
        ...
        className="glass-panel relative mx-auto flex max-w-7xl items-center justify-between rounded-2xl px-4 py-3 md:px-6"
      >
        <a href="#hero" className="font-display text-sm font-semibold uppercase tracking-[0.16em] text-slate-800 md:text-base">
          Rayan Studio
        </a>
        ...
        <ul className="hidden items-center gap-5 lg:flex">
          {links.map((link) => (
            <li key={link.href}>
              <a href={link.href} className="text-sm font-medium text-slate-600 transition-colors hover:text-slate-900">
                {link.label}
              </a>
            </li>
          ))}

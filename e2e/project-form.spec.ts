...
import { acceptAnalytics, mockProjectError, mockProjectSuccess } from "./fixtures";
...
async function walkToFinalStep(page: Page) {
  await page.getByRole("radio", { name: "Application / SaaS" }).check();
  await page.getByRole("button", { name: "Suivant" }).click();
  await page.getByRole("radio", { name: "Produit existant" }).check();
  await page.getByRole("button", { name: "Suivant" }).click();
  await page.getByLabel("Objectif du projet").fill("Faire évoluer une plateforme métier existante.");
  await page.getByRole("button", { name: "Suivant" }).click();
  await page.getByRole("radio", { name: "1-3 mois" }).check();
  await page.getByRole("button", { name: "Suivant" }).click();
...
test.describe("project intake", () => {
  ...
  test("EN smoke: natural labels on the canonical route", async ({ page }) => {
    ...
    await page.getByRole("radio", { name: "MVP", exact: true }).check();
    await page.getByRole("button", { name: "Next" }).click();
    await page.getByRole("radio", { name: "An idea" }).check();
    await page.getByRole("button", { name: "Next" }).click();

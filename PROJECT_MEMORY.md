...
- **CI de prospection hebdomadaire (branche `prospection-v1`, GO utilisateur avec amendements)** : nouveau pipeline `scripts/prospection/` (Node, package scripts) + routes Next `/api/unsubscribe` et `/api/webhooks/resend`. Détails complets : `scripts/prospection/README.md`. Points clés :
  ...
  - Review round 2 (REQUEST CHANGES) : 2 bloquants corrigés (commit 7da791c) — la reprise des RESERVED est désormais soumise à TOUS les garde-fous (refus total si SEND_ENABLED!=true : l'arrêt d'urgence stoppe aussi les reprises ; garde-fous + suppression revérifiés par message, message non autorisé reste RESERVED) ; le contact conforme est persisté juste après l'audit et AVANT le passage ENRICHED (un échec LLM/garde-fou ne peut plus le perdre). Important : ligne d'audit `is_summary` (migration 003) portant les signaux fusionnés, seule rechargée à la réutilisation ; dry-run sans duplication de brouillons (draftsExisting) ; ingest ATTACH enrichit canonical_domain NULL (conflit = fail closed) ; title/meta retirés du payload LLM (signaux + URLs de preuve uniquement). DNS rebinding : reporté à l'avant-activation avec accord du reviewer. 123 tests, CI verte.
...
- **P0 branche `fix/legal-pages`** :
  ...
  - Suite (meme jour, reponses utilisateur) : mentions legales completees avec les valeurs de pont-facturx.com/legal/mentions (Micro-entrepreneur, SIRET 980 099 766 00019, TVA non assujetti, siege « 8 allee du pre, 72190 Saint-Pavace » ; pont-facturx.com affiche encore l'adresse sans code postal ni ville, a corriger la-bas) ; choix cookies expire apres 6 mois (`saveConsent`/`readConsent` dans `analytics.ts`, cle `rayan_cookie_consent_v1_at`, choix anterieurs horodates a la premiere lecture, test ajoute) ; `HomePage.tsx` legacy supprime. 145 tests, build 170 pages, curl OK.
- **Prospection : decision de ne PAS reecrire** (2026-09-23, apres resume complet du pipeline `scripts/prospection/` et brainstorm). Constats : pipeline jamais active (cron echoue sur `DATABASE_URL manquant`, Neon jamais cree), **Resend interdit le cold email dans son AUP** (bloquant avant toute activation), bug confirme `crawl.js:59` (regex copyright prend la premiere annee, « © 2010-2026 » → 2010 → refonte a tort), `newestCopyrightYear` non transmis au LLM. Ordre retenu : (1) debloquer le parcours missions existant `freelance-opportunities` (l'utilisateur n'a jamais ouvert un rapport : blocage = distribution, pas selection ; 86/87 annonces rejetees par le filtre remote), (2) trancher le canal cold email (Resend exclu), (3) seulement ensuite pilote refonte 4 semaines sur un segment avec brouillons approuves a la main et 3 fixes cibles. Le web premium reste un parcours secondaire, pas a abandonner.
  - Sous-projet 1 livre (branche `feat/missions-report-email`) : statut remote `onsite_or_hybrid` scinde en `onsite` / `hybrid` ; `buildReport` ajoute `toVerify` (statuts `unknown` + `hybrid`, top 10 par score, avec brouillon) et `remoteRejected` (echantillon compact de 20 pour calibrer) ; `renderReportEmail` + `sendReportEmail` (Gmail, envoi seulement si candidates ou a verifier non vides, `no_credentials` sinon) ; workflow passe `GMAIL_USER`, `GMAIL_APP_PASSWORD`, `FREELANCE_REPORT_TO`. TDD : 5 tests rouges puis verts, suite scripts 127/127. **A faire cote utilisateur : creer la variable de depot `FREELANCE_REPORT_TO`** (sinon envoi a `GMAIL_USER`) et verifier que les secrets Gmail du workflow outreach existent toujours.
  - Reste a faire / non verifie : **aucun job de purge n'existe pour la duree de 3 ans annoncee en prospection** (a creer avant activation `SEND_ENABLED`) ; retention GA4 verifiee sur capture utilisateur : evenements 2 mois, donnees utilisateur 14 mois, « Reinitialiser lors d'une nouvelle activite » active (recommandation : le desactiver pour que « 14 mois au maximum » soit strictement vrai, sinon preciser « a compter de la derniere visite » dans la politique). Prospection : l'utilisateur prevoit de revoir le script ; integrer la purge 3 ans a cette revision.

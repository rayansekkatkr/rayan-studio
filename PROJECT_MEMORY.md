...
- **P0 branche `fix/legal-pages`** :
  ...
  - Suite (meme jour, reponses utilisateur) : mentions legales completees avec les valeurs de pont-facturx.com/legal/mentions (Micro-entrepreneur, SIRET 980 099 766 00019, TVA non assujetti, siege « 8 allee du pre » tel quel, **sans code postal ni ville sur les deux sites**) ; choix cookies expire apres 6 mois (`saveConsent`/`readConsent` dans `analytics.ts`, cle `rayan_cookie_consent_v1_at`, choix anterieurs horodates a la premiere lecture, test ajoute) ; `HomePage.tsx` legacy supprime. 145 tests, build 170 pages, curl OK.
  - Reste a faire / non verifie : **aucun job de purge n'existe pour la duree de 3 ans annoncee en prospection** (a creer avant activation `SEND_ENABLED`) ; retention 14 mois a verifier dans la propriete Google Analytics 4 (Admin > Data retention) ; completer l'adresse du siege (code postal, ville).
